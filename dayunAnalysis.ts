...
export function analyzeDaYun(
  ...
  // Bug fix（V2.0 审计发现）：显式传入 sixLines.year.gan（已在 calculator.ts 中做过立春分界校正），
  // 避免 generateDaYun 内部按公历年份重新计算年干造成的口径不一致。
  const yearGan = sixLines.year.gan as HeavenlyStem
  const rawSteps = generateDaYun(birthDate, dayGan, gender, monthZhi, 8, yearGan)
